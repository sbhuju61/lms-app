...
public class BorrowerDAO extends BaseDAO{
	...
	public List<Book> getAvailableBooksByBranch(Branch branch) throws ClassNotFoundException, SQLException {
		...
		return read("select * from tbl_book inner join tbl_book_copies on tbl_book.bookID = tbl_book_copies.bookID   where branchID = ?;",
	...
	List extractData(ResultSet rs) throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			...
			//borrower.setAvailableBooks(bdao.readFirstLevel("select * from tbl_book inner join "
				//	+ "tbl_book_copies on tbl_book.bookID = tbl_book_copies.bookID where noOfCopies > 0;", null));
